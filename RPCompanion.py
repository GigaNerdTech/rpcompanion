...
async def commit_sql(sql_query, params = None):
    ...
    try:
        connection = mysql.connector.connect(host='localhost', database='RPCompanion', user='REDACTED', password='REDACTED')    
...
#            result = await commit_sql("""INSERT INTO CharacterProfiles (SELECT * FROM UnapprovedCharacterProfiles WHERE Id=%s);""", (message.content,))
            ...
            if current_field == 0:
                ...
                role_found = False
                for role in current_guild.roles:
                    if role.name == message.content or message.content == 'None':
                        role_id = role.id
                        role_found = True
                if not role_found:
            ...
            if current_field == 0:
                ...
                for player in players:
                    for role in player.roles:
                        if role.id == guild_settings[dm_tracker[message.author.id]["server_id"]]["NPCRole"]:
 ...
 #               fields // "**VENDOR FIELDS**\n\n`VendorName:` The name of the vendor as it appears in buying items.\n`ItemList:` A comma delimited list of item IDs available for purchase.\n"
        ...
        if command == 'deleteall':
            ...
            if int(number_of_dice) > 100:
                await reply_message(message, "You can't roll more than 100 dice!")
                return
            response = "**Dice roll:**\n\n"
            sum = 0
            ...
            if records:
                ...
                for row in records2[1:]:
                    ...
                    if user is None:
                        ...
                        username = user.name                
                        response = response + row[0] + " - " + username + "\n"
                await reply_message(message, response)
                return    
            ...
            for row in records:
                ...
                if user is None:
                    ...
                    username = user.name
                response = response + row[0] + ", Player: `" + username + "`\n"
            await reply_message(message, response)
            ...
            if records:
                ...
                if len(records) < 1:
                    ...
                    return
                for row in records:
                    username = client.get_user(id=int(row[9])).name
